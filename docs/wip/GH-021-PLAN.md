...
- At the alarm time, select the Alarm view, show the current time full-screen,
  request the exact Hue scene `Warm low`, and loop one selected endpoint-local
  audio file until the user dismisses the alarm.
- Add a deliberate two-step sleep flow that arms the iMac's hardware RTC and
  suspends the iMac until the alarm time.
- Make endpoint-local alarm audio selectable without rebuilding the client or
  changing code.
...
- Snooze, gradual light fades, volume ramps, playlists, streaming audio,
  microphone input, voice control, or touch controls.
...
- A general scheduler, automation engine, endpoint command API, or configurable
  action sequence beyond choosing one endpoint-local alarm MP3.
...
- [ ] Alarm editing is keyboard-only. Left and Right select hours or minutes;
  ...
  `00–23` and `00–59`. Minutes use five-minute increments, and holding either
  arrow repeats the selected-field action.
...
- [ ] The iMac starts the selected endpoint-local alarm file when it observes
  the matching ringing alarm and loops it until dismissal or disarming. Reload,
  reconnect, repeated ringing snapshots, and Alarm view unmounts cannot leave
  overlapping or stale players.
...
- [ ] The alarm catalogs regular `.mp3` files in
  `/var/lib/cortex-endpoint/alarm-audio`, readable by `cortex-endpoint`; the
  repository does not contain copyrighted or personal alarm audio.
- [ ] Pressing `A` while disarmed opens the keyboard-only selector; Up/Down
  move the selection and Enter applies it. The catalog exposes no arbitrary
  path, upload API, or media service.
- [ ] `scp <local-mp3> imac:/var/lib/cortex-endpoint/alarm-audio/` adds an
  option with no source edit, client rebuild, coordinator restart, or full
  provisioning.
...
- [ ] A bounded physical qualification proves that the real iMac enters
  suspend-to-idle and resumes from its `rtc_cmos` alarm at the requested UTC
  instant without corrupting the legacy Radeon display state.
...
- [ ] During suspend-to-idle, Chromium, UxPlay, Raspotify, PulseAudio, and
  endpoint scripts perform no work while the kernel waits for the RTC wake
  interrupt.
...
- [ ] A deployed manual pass proves awake firing, suspend-to-idle and RTC wake,
  `Warm low`, full-screen time, selectable audio,
...
- Extend the exact-origin endpoint bridge with the bounded sleep operation,
  install the narrow privileged RTC helper, and qualify suspend-to-idle/resume
  on the real iMac.
...
## 5. GH-021: Select The Alarm Audio

- Add and document one bounded endpoint-local MP3 catalog, keyboard selector,
  initial-file installer, ownership boundary, and direct `scp` workflow.
- This is atomic because choosing the user-owned alarm sound has a separate
...
- Proposed approach: Left/Right changes fields; Up increments; Down decrements;
  minutes use five-minute increments and held arrows repeat;
  two number presses replace the selected value; field changes and a short
  timeout clear the numeric buffer.
- Why: The attached keyboard is the room control, and held Alt allows precise
  one-minute correction without slowing ordinary five-minute editing.
...
### H3 - Keep Audio Selection Endpoint-Local And Single-Owner

- Decision: Loop one selected iMac-local MP3 through a small endpoint player
  controlled by the existing exact-origin bridge.
- Proposed approach: Catalog only bounded regular MP3 names, persist one
  selection, record one owned player PID, reject overlap, and stop it on
  dismissal, disarming, or session cleanup.
...
- Alternatives: Bundle audio in the client; serve it from the ThinkPad; invoke
  Spotify; synthesize a tone; allow an arbitrary path or playlist.
